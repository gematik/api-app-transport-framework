...
Expression: "Bundle.entry.resource.where($this.is(Bundle).not()).descendants().ofType(Reference).reference.where($this.startsWith('#').not()).all((%resource.entry.fullUrl.join('|') + '|').contains($this + '|'))"
Severity: #error
